...
test("buildRepairPlan enumerates DNS hosts and the CA + proxy teardown steps", async () => {
  const plan = await manager.buildRepairPlan();
...
test("buildRepairPlan reuses collectManagedHosts (same managed host set)", async () => {
  const plan = await manager.buildRepairPlan();
  assert.deepEqual(
    [...plan.dnsHostsToRemove].sort(),
    [...(await manager.collectManagedHosts())].sort(),

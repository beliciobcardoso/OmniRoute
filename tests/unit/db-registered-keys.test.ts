...
test("registered keys honor idempotency and list filters", async () => {
  ...
  if ("idempotencyConflict" in first) throw new Error("unexpected conflict");
  ...
  if (!("idempotencyConflict" in second)) throw new Error("expected idempotency conflict");
...
test("registered keys enforce provider and account quota limits", async () => {
  ...
  if ("idempotencyConflict" in created) throw new Error("unexpected conflict");

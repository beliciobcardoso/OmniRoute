...
test("translateRequest replays cached reasoning-only messages when interleaved field is reasoning_content", async () => {
...
test("translateRequest injects thinking block into Claude-format messages for Kimi K2 reasoning models", async () => {
  await clearReasoningCacheAll();
  clearModelsDevCapabilities();
  saveModelsDevCapabilities({
    "kimi-coding": {
...
test("translateRequest injects placeholder thinking block for Claude-format Kimi K2 on cache miss", async () => {
  await clearReasoningCacheAll();
  clearModelsDevCapabilities();
  saveModelsDevCapabilities({
    "kimi-coding": {

...
export async function getComboForecastUsageRows(opts: {
  ...
  if (isPostgres()) {
    ...
    const conditions = [
      sql`combo_name IS NOT NULL`,
      sql`combo_name != ''`,
      sql`timestamp >= ${opts.since}`,
    ];
    if (opts.until) conditions.push(sql`timestamp <= ${opts.until}`);
    if (opts.comboName) conditions.push(sql`combo_name = ${opts.comboName}`);
    const whereClause = sql.join(conditions, sql` AND `);

    const query = sql<ComboForecastUsageSqlRow>`
      SELECT
    ...
    const result = await query.execute(getKyselyDb());
